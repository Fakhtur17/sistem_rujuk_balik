{{-- resources/views/pic_apoteks/edit.blade.php --}}
<x-app-layout>
        ...
        <div class="container mx-auto px-4 max-w-2xl">
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8 mb-6">
                <div class="flex items-center space-x-4 mb-6">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-800">Edit PIC Apotek</h1>
                        <p class="text-gray-600">Perbarui informasi Person In Charge Apotek</p>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8">
                <form action="{{ route('pic-apoteks.update', ['picPrb' => $data->pic_prb_id, 'id' => $data->id]) }}" method="POST" class="space-y-6" id="apotekForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 pt-6">
                        ...
                        <a href="{{ route('pic-apoteks.index', $data->pic_prb_id) }}" 
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                ...
                <div class="bg-purple-50 border border-purple-200 rounded-xl p-6">
                    <div class="flex items-start space-x-3">
                        <i class="fa fa-history text-purple-500 text-lg mt-1"></i>
                        <div>
                            <h4 class="font-semibold text-purple-800 mb-3">Informasi Update</h4>
                            <ul class="text-sm text-purple-700 space-y-2">
                                <li class="flex items-start">
                                    <i class="fa fa-check-circle text-purple-500 mr-2 mt-0.5 text-xs"></i>
                                    <span>Perubahan data akan diverifikasi ulang oleh sistem</span>
                                </li>
                                <li class="flex items-start">
                                    <i class="fa fa-check-circle text-purple-500 mr-2 mt-0.5 text-xs"></i>
                                    <span>Pastikan nomor HP yang dimasukkan masih aktif</span>
                                </li>
                                <li class="flex items-start">
                                    <i class="fa fa-check-circle text-purple-500 mr-2 mt-0.5 text-xs"></i>
                                    <span>Data akan disinkronisasi dengan sistem BPJS</span>
                                </li>
                                <li class="flex items-start">
                                    <i class="fa fa-check-circle text-purple-500 mr-2 mt-0.5 text-xs"></i>
                                    <span>Backup data lama akan disimpan untuk audit</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="bg-pink-50 border border-pink-200 rounded-xl p-6">
                    <div class="flex items-start space-x-3">
                        <i class="fa fa-exclamation-triangle text-pink-500 text-lg mt-1"></i>
                        <div>
                            <h4 class="font-semibold text-pink-800 mb-3">Perhatian Penting</h4>
                            <div class="space-y-3">
                                <div class="flex items-start space-x-3">
                                    <div class="w-6 h-6 bg-pink-200 rounded-full flex items-center justify-center text-pink-700 text-xs font-bold flex-shrink-0 mt-0.5">!</div>
                                    <span class="text-sm text-pink-700">Perubahan nama apotek memerlukan verifikasi dokumen</span>
                                </div>
                                <div class="flex items-start space-x-3">
                                    <div class="w-6 h-6 bg-pink-200 rounded-full flex items-center justify-center text-pink-700 text-xs font-bold flex-shrink-0 mt-0.5">!</div>
                                    <span class="text-sm text-pink-700">Update PIC dapat mempengaruhi akses sistem</span>
                                </div>
                                <div class="flex items-start space-x-3">
                                    <div class="w-6 h-6 bg-pink-200 rounded-full flex items-center justify-center text-pink-700 text-xs font-bold flex-shrink-0 mt-0.5">!</div>
                                    <span class="text-sm text-pink-700">Koordinasikan dengan tim IT untuk perubahan besar</span>
                                </div>
                                <div class="flex items-start space-x-3">
                                    <div class="w-6 h-6 bg-pink-200 rounded-full flex items-center justify-center text-pink-700 text-xs font-bold flex-shrink-0 mt-0.5">!</div>
                                    <span class="text-sm text-pink-700">Simpan salinan data lama sebagai cadangan</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-gradient-to-r from-purple-100 to-pink-100 border border-purple-200 rounded-xl p-4 mt-6">
                <div class="flex items-center justify-between">
                    <div class="flex items-center text-purple-800">
                        ...
                        <span class="font-medium">Pastikan semua data sudah benar sebelum memperbarui</span>
                    </div>
    ...
    <style>
        ...
        .form-group input:not(:placeholder-shown) + div .fa-check-circle {
            opacity: 1;
        }
    </style>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize check icons for pre-filled data
            inputs.forEach(input => {
                const checkIcon = document.getElementById(input.name + '_check');
                if (input.value.trim().length > 0 && checkIcon) {
